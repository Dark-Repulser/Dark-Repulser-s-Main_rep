...
# Función para buscar un ticket por su número y obtener el ID y contactId
def buscar_ticket_por_numero(ticket_number):
    ...
    if response.status_code == 200:
        ...
        if 'data' in data and len(data['data']) > 0:
            ticket_data = data['data'][0]
            return {
                "Ticket Number": ticket_number,
                "Ticket ID": ticket_data.get('id'),
                "Contact ID": ticket_data.get('contactId')
            }
        else:
            print(f"No se encontró ticket con el número: {ticket_number}")
            return {"Ticket Number": ticket_number, "Ticket ID": None, "Contact ID": None}
    else:
        print(f"Error al buscar el ticket {ticket_number}: {response.status_code}")
        print(response.text)
        return {"Ticket Number": ticket_number, "Ticket ID": None, "Contact ID": None}

# Función principal para procesar todos los tickets del CSV
def procesar_tickets(test_csv, output_csv):
    df = pd.read_csv(test_csv)
    resultados = []  # Lista para almacenar los resultados

    for index, row in df.iterrows():
        ticket_number = row['ticketID']  # Número de ticket del CSV
        print(f"Buscando ticket {ticket_number}...")

        # Buscar el ticket y obtener la información
        resultado = buscar_ticket_por_numero(ticket_number)
...
csv_file = 'TEST.csv'  # Archivo CSV con los números de ticket
output_csv = 'resultados.csv'  # Archivo CSV de salida

# Procesar los tickets y exportar los resultados
procesar_tickets(csv_file, output_csv)
