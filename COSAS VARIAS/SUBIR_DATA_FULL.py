...
def update_lead_campaign(lead_id, row, access_token):
    ...
    # Mapeo condicional de correo y teléfono según el destino
    if periodo == "25C06":  # Diplomado
        # Usar campos específicos para Continua
        data_dict["Correo_electr_nico_Continua"] = row.get("Correo_electrónico_agregado", "")
        data_dict["Tel_fono_Continua"] = str(row.get("Teléfono", ""))
        logging.info(f"Lead {lead_id} es Diplomado - usando campos Continua")
    else:  # Carrera
        # Usar campos estándar
        data_dict["Email"] = row.get("Correo_electrónico_agregado", "")
        data_dict["Phone"] = str(row.get("Teléfono", ""))
        logging.info(f"Lead {lead_id} es Carrera - usando campos estándar")
...
def create_lead_from_csv_row(row, access_token):
    ...
    if periodo == "25C06":  # Diplomado
        ...
        data_dict["Correo_electr_nico_Continua"] = row.get("Correo_electrónico_agregado", "")
        data_dict["Tel_fono_Continua"] = str(row.get("Teléfono", ""))
        logging.info(f"Nuevo lead es Diplomado - usando campos Continua")
    else:  # Carrera
        # Usar campos estándar
        data_dict["Email"] = row.get("Correo_electrónico_agregado", "")
        data_dict["Phone"] = str(row.get("Teléfono", ""))
        logging.info(f"Nuevo lead es Carrera - usando campos estándar")
...
def process_csv(file_path="ARCHIVO FINAL FULL_v3.csv", access_token=None):
...
# Función principal
def main(file_path="ARCHIVO FINAL FULL_v3.csv"):
...
if __name__ == "__main__":
    main()
    #uwu con amor david romero zohito
