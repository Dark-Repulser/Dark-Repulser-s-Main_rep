...
def process_records_binary(df, start, end):
    ...
    if response.status_code == 200:
        ...
        if hora_modificacion_excel >= hora_modificacion_crm:
            ...
            estado = "tilin"  # row['Estado']
            subestado = "x"   # row['sub estado']
            subestadoII = "x" # row["sub estado II"]
            subestadoIII = "xd" # row["sub estado III"]
            etapa = "xd" 
            update_data = {
                "data": [{
...
# Función principal que ejecuta el proceso y guarda archivos
def job():
    global no_actualizados
    no_actualizados = []  # Reiniciar la lista de registros no actualizados
    ...
    # Filtrar registros procesados (aquellos que están en no_actualizados)
    df_processed = df[~df['Id'].isin([row['Id'] for row in no_actualizados])]

    # Guardar el archivo original sin los registros procesados
    df_processed.to_csv('testo_procesado.csv', index=False)

    # Guardar registros no actualizados en CSV
    no_actualizados_df = pd.DataFrame(no_actualizados)
    no_actualizados_df.to_csv("test_one.csv", index=False)

    print("Proceso completado. Archivos generados: 'test_one.csv' con registros no actualizados y 'testo_procesado.csv' sin registros procesados.")

# Programar el trabajo para que se ejecute cada minuto
schedule.every(1).minutes.do(job)

print("Scheduler iniciado. Esperando para ejecutar el trabajo cada minuto...")
